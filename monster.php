...
if ($numpages != 1) {
	if ($numpages <= $page_max){
		...
		if ($page <= 4) {
			echo ($page == 1)?'<span class="current">1</span>' . " ":'<a href="monster.php?p=1">1</a> ';
			echo ($page == 2)?'<span class="current">2</span>' . " ":'<a href="monster.php?p=2">2</a> ';
...
$query = $db->execute("select `id`, `username`, `level` from `monsters` order by `level` desc limit ?,?", array($begin, $limit));
while($member = $query->fetchrow()) {
	...
	echo "<td><a href=\"battlem.php?act=attack&username=" . $member['username'] . "\">" . $lang['keyword_attack'] . "</a></td>\n";
	echo "</tr>\n";
}
?>
</table>
